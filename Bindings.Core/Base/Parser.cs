...
namespace Bindings.Core.Base
{
    public abstract class Parser
    {
        ...
        protected string ReadQuotedString()
        {
            ...
            if (IsComplete)
            {
                throw new BindingException("Error parsing string indexer - unterminated in text {0}", FullText);
            }
        ...
        protected uint ReadUnsignedInteger()
        {
            ...
            {
                throw new BindingException("Unable to parse integer text from {0} in {1}", integerText, FullText);
            }
            return index;
        }
            ...
            switch (currentChar)
            {
                ...
                case 'U':
                    ...
                    if (firstFourDigits != "0000")
                        throw new BindingException("\\U unicode character does not start with 0000 in {1}", FullText);
                    return ReadFourDigitUnicodeCharacter();

                default:
                    throw new BindingException("Sorry we don't currently support escaped characters like \\{0}",
                                           currentChar);
            }
        }
        ...
        private char ReadFourDigitUnicodeCharacter()
        {
            ...
            if (number > ushort.MaxValue)
                throw new BindingException("\\u unicode character {0} out of range in {1}", number, FullText);
            return (char)number;
        }
        ...
        private string ReadNDigits(int count)
        {
            ...
            {
                if (IsComplete)
                    throw new BindingException("Error while reading {0} of {1} digits in {2}", i + 1, count, FullText);

                var currentChar = CurrentChar;
                if (!char.IsDigit(currentChar))
                    throw new BindingException("Error while reading {0} of {1} digits in {2} - not a char {3}", i + 1, count,
        ...
        protected object ReadValue()
        {
            ...
            if (!TryReadValue(AllowNonQuotedText.Allow, out toReturn))
                throw new BindingException("Unable to read value");
            return toReturn;
        }
        ...
        protected bool TryReadValue(AllowNonQuotedText allowNonQuotedText, out object value)
        {
            ...
            if (IsComplete)
            {
                throw new BindingException("Unexpected termination while reading value in {0}", FullText);
            }
            ...
            while (!IsComplete)
            {
                ...
                if (currentChar == '.')
                {
                    if (decimalPeriodSeen)
                        throw new BindingException("Multiple decimal places seen in number in {0} at position {1}", FullText,
                                               CurrentIndex);
        ...
        protected ValueType NumberFromText(string numberText, bool decimalPeriodSeen)
        {
            if (decimalPeriodSeen)
            {
                ...
                throw new BindingException("Failed to parse double from {0} in {1}", numberText, FullText);
            }
            else
            {
                ...
                throw new BindingException("Failed to parse Int64 from {0} in {1}", numberText, FullText);
            }
        }
        ...
        protected object ReadEnumerationValue(Type enumerationType, bool ignoreCase = true)
        {
            ...
            {
                throw new BindingException(exception, "Problem parsing {0} from {1} in {2}", enumerationType.Name, name, FullText);
            }
        }
        ...
        protected string ReadValidCSharpName()
        {
            ...
            {
                throw new BindingException("PropertyName must start with letter - position {0} in {1} - char {2}",
