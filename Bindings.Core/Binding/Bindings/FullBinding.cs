...
{
    public class FullBinding
        : Binding, IUpdateableBinding
    {
        private ISourceStepFactory SourceStepFactory => BindingSingletonCache.Instance.SourceStepFactory;

        private ITargetBindingFactory TargetBindingFactory => BindingSingletonCache.Instance.TargetBindingFactory;

        private readonly BindingDescription _bindingDescription;
        private ISourceStep _sourceStep;
        private ITargetBinding _targetBinding;
        private readonly object _targetLocker = new object();

        private object _dataContext;
        private EventHandler _sourceBindingOnChanged;
        private EventHandler<TargetChangedEventArgs> _targetBindingOnValueChanged;

        private object _defaultTargetValue;
        private CancellationTokenSource _cancelSource = new CancellationTokenSource();
        private IMainThreadAsyncDispatcher dispatcher => BindingSingletonCache.Instance.MainThreadDispatcher;
        ...
        {
            if (value == BindingConstant.DoNothing || cancel.IsCancellationRequested)
                return;
        ...
        protected BindingMode ActualBindingMode
        {
            get
            {
                var mode = _bindingDescription.Mode;
                if (mode == BindingMode.Default && _targetBinding != null)
